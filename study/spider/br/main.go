...
func FormatData() error {
	...
	if err := WriteCsvFile(countryName+"_with_code.csv", func(w *csv.Writer) error {
		...
		for {
			...
			province, city, area := data[0], data[2], data[3]
			provinceCode := "BR-" + data[1]
...
func FetchAllCity() error {
	...
	c.OnHTML(".view-uf-list", func(h *colly.HTMLElement) {
		h.DOM.Find("table tbody tr").Each(func(i int, s *goquery.Selection) {
			...
			m.Store(href, Area{
				Code: fmt.Sprintf("BR-%s", provinceCode),
				Name: city,
				Link: href,
			})
		})
	...
	c.Visit("https://bra.youbianku.com/zh-hans/uf-list")
...
func fetchAllAreaAndZip(ctx context.Context) (chan []string, chan string) {
	...
	go func() {
		...
		// 	q.AddURL(fmt.Sprintf("https://bra.youbianku.com%s", url.Link))
		// }
		...
		for {
			...
			q.AddURL(fmt.Sprintf("https://bra.youbianku.com%s?p=%s&pc=%s", data[3], data[0], data[1]))
		}
		...
		c.OnHTML(".view-admin2-admin3", func(h *colly.HTMLElement) {
			query := h.Request.URL.Query()
			province := query.Get("p")
			provinceCode := query.Get("pc")

			var hasData bool
			h.DOM.Find("table tbody tr").Each(func(i int, s *goquery.Selection) {
				hasData = true
				city := strings.TrimSpace(s.Find("td.views-field-field-locality").Text())
				link := s.Find("td.views-field-field-area a[href]")
				area := strings.TrimSpace(link.Text())
				// href, _ := link.Attr("href")
				ch <- []string{province, provinceCode, city, area}
			})
