...
func FormatData() error {
	...
	if err := WriteCsvFile(countryName+"_with_code.csv", func(w *csv.Writer) error {
		...
		for {
			...
			d := strings.Split(province, "/")
			provinceCode, ok := ProvinceMap[d[0]]
			if !ok {
				arr[province] = nil
			}
...
func FetchAllCity() error {
	...
	c.OnHTML(".view-region1-text .views-field-title a[href]", func(h *colly.HTMLElement) {
		link := h.Attr("href")
	...
	c.OnHTML(".view-region1-text li.pager-next", func(h *colly.HTMLElement) {
	...
	c.Visit("https://kor.postcodebase.com/zh-hans/region1-text")
...
func fetchAllAreaAndZip(ctx context.Context) (chan []string, chan string) {
	...
	go func() {
		...
		for {
			...
			q.AddURL(fmt.Sprintf("https://kor.postcodebase.com%s?p=%s", data[2], data[0]))
		}
		// q.AddURL("https://kor.postcodebase.com/zh-hans/region2/eunpyeong-gu%EC%9D%80%ED%8F%89%EA%B5%ACseoul%EC%84%9C%EC%9A%B8?p=Seoul/서울")
		...
		c.OnHTML(".view-region2-city", func(h *colly.HTMLElement) {
			...
			h.DOM.Find("table tbody tr").Each(func(i int, s *goquery.Selection) {
				...
				link := s.Find("td.views-field-field-city-text a[href]")
				city := strings.TrimSpace(link.Text())
				// href, _ := link.Attr("href")
				ch <- []string{p, province, city}
			})
